...
def _get_key_material(certificate):
    ...
    """
    pubKey = (
        cert_finder._get_tbs_certificate(certificate)
        .getComponentByName("subjectPublicKeyInfo")
        .getComponentByName("subjectPublicKey")
    )

    signing_alg = str(
        cert_finder._get_tbs_certificate(certificate).getComponentByName("subjectPublicKeyInfo").getComponentByName("algorithm")
    )
        ...
        if auth_attributes is None:
            ...
            for attr in auth_attributes:
                ...
                if type == MESSAGE_DIGEST_KEY:
                    ...
                    if value != calculated:
                        raise Exception(
                            "Digest in authenticated attributes differs\
...
def verify_msg(asn1_pkcs7_msg):
    ...
    msg = message_content.getComponentByName("content").getComponentByName("signed_content").getContentValue()
...
def verify_qts(asn1_qts):
    ...
    msg = qts_content.getComponentByName("encapsulatedContentInfo").getComponentByName("eContent")._value
    return _verify_data(msg, certificates, signer_infos)
