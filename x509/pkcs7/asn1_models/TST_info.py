...
class MessageImprint(univ.Sequence):
    componentType = namedtype.NamedTypes(
        namedtype.NamedType("algId", AlgorithmIdentifier()), namedtype.NamedType("imprint", univ.OctetString())
    )
...
class Accuracy(univ.Sequence):
    componentType = namedtype.NamedTypes(
        ...
        namedtype.OptionalNamedType(
            "milis", univ.Integer().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0x0))
        ),
        namedtype.OptionalNamedType(
            "micros", univ.Integer().subtype(implicitTag=tag.Tag(tag.tagClassContext, tag.tagFormatSimple, 0x1))
        ),
    )
